...
- Current text set by the project owner on 2026-07-07 (rename from the earlier
  `f5-mode`; naming decision recorded in STATE.md). Behavioral doctrine only —
  no commands or version-pinned facts that can drift.
- Changing any rule here is a doctrine change: route it through
  `bbb-change-control` (Class 4).
- Re-verify sibling cross-references (`bbb-eval-loop`,
  `bbb-verification-and-evidence`, `bbb-subagent-orchestration`) with
  `ls .claude/skills/` from the library root.
